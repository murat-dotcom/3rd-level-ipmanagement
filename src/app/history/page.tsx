...
export default function HistoryPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto space-y-6">
      ...
      {/* Comparison panel */}
      {comparing && a && b && (
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-primary space-y-4">
          <h2 className="font-bold text-primary">比較結果</h2>
          <div className="grid grid-cols-3 gap-2 text-center text-sm">
            <div className="font-medium text-slate-500">項目</div>
            <div className="font-bold text-primary">{a.date}</div>
            <div className="font-bold text-accent">{b.date}</div>

            <div className="text-slate-600 dark:text-slate-400">スコア</div>
            ...
            <div className="text-slate-600 dark:text-slate-400">種別</div>
            <div>{a.type === 'gakka' ? '学科' : '実技'}</div>
            <div>{b.type === 'gakka' ? '学科' : '実技'}</div>

            <div className="text-slate-600 dark:text-slate-400">時間</div>
            <div>{formatTime(a.timeSpent)}</div>
            <div>{formatTime(b.timeSpent)}</div>

            <div className="text-slate-600 dark:text-slate-400">間違い数</div>
            <div className="text-error">{a.wrongQuestionIds.length}</div>
            <div className="text-error">{b.wrongQuestionIds.length}</div>

            {/* Score difference */}
            <div className="col-span-3 pt-2 border-t border-slate-200 dark:border-slate-700">
              {(() => {
                const pctA = Math.round((a.score / a.total) * 100);
                const pctB = Math.round((b.score / b.total) * 100);
                const diff = pctB - pctA;
                return (
                  <p className={`font-bold ${diff > 0 ? 'text-success' : diff < 0 ? 'text-error' : 'text-slate-500'}`}>
          ...
          {history.map((quiz, i) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left bg-white dark:bg-slate-800 rounded-xl p-4 border transition-colors ${
                  isSelected ? 'border-primary ring-2 ring-primary/20' : 'border-slate-200 dark:border-slate-700 hover:border-primary'
                }`}
              >
                <div className="flex justify-between items-center">
                  <div>
                    <span className="text-sm font-medium">{quiz.date}</span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-slate-400">{formatTime(quiz.timeSpent)}</span>
                    {isSelected && (
      ...
      <button
        onClick={() => router.push('/')}
        className="w-full py-3 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-medium"
      >
        ダッシュボードへ
      </button>
    </div>
